...
%properties. The surface exchange kinetics are explicitly considered 
%(i.e. sg-eq condition not valid)
...
%- Make sure that the Equations_DarkLight.m file is stored in the same
...
v_coeff = 1;         %Set to 0 to if vacancies are redox inactive, 1 otherwise (within this version v_coeff can be 1 only if i_coeff is 1 too)
...
Gext = SunsEq*0.023/q/5e-5/conc_*time_; %External generation term, here considered constant in the material

krad = 1e-11*conc_*time_;               %Radiative recombination constant 
Cn = 1e-28*conc_^2*time_;               %Auger coefficients
Cp = 1e-28*conc_^2*time_;

%Bimolecular generation-recombination of vacancies and interstitials is
%considered for the anti-Frenkel disorder
kion_rec = 1e-15*conc_*time_;           %Bimolecular ionic defects recombination rate constant (kf_aF in the manuscript)
KaF = 1e32/conc_^2;                     %anti-Frenkel disorder mass action constant

PI2_IP = 1e-2/pressure_;                %PI2 at the boundary between the I and P region (p = [I_i^']). Estimate from conductivity measurements assuming mobility for n and p of 10 cm^2 V^-1 s^-1 and for ion 1e-9 cm^2 V^-1 s^-1
PI2_NI = PI2_IP/(KaF/ni^2)^2;           %PI2 at the boundary between the N and I region (n = [V_I^.])
PI2i = PI2_IP*ni^2/KaF;                 %PI2 corresponding to the intrinsic condition p = n, [I_i^'] = [V_I^.]
...
kb_sg_i = kf_sg_i/K_sg_i;
kb_sg_v = kf_sg_v/K_sg_v;
...
%decades in P(I2) for the P and the N region respectively
...
VIx0dark = K_sg_v/sqrt(PI2_NI*10^(-Decades_pI2_Nregion));
Iix0dark = sqrt(PI2_NI*10^(-Decades_pI2_Nregion))/K_sg_i;

if Include_dark == 1
    
    xsol = [p0dark n0dark VI0dark Ii0dark VIx0dark Iix0dark];
    xsolall_dark = zeros(N,6);
    xguessall_dark = zeros(N,6);

    for i = 1:N 
%         i
        par = [pI2(i), kf_n_i, kf_p_i, kb_n_i, kb_p_i, kf_n_v, kf_p_v, kb_n_v, kb_p_v, kf_sg_i, kf_sg_v, kb_sg_i, kb_sg_v, KaF, kion_rec, ni, n1, p1, taun, taup, 0, krad, Cn, Cp];
        fun = @(x)Equations_DarkLight(x, par, Rec_Model, Redox_Model);
        if i<3
            % Assuming a slope of 1/4 and -1/4 the initial guesses are
            % calculated for the first 2 data points
            x0 = [xsol(1)*10^((i-1)*logstep/4) xsol(2)*10^(-(i-1)*logstep/4) xsol(3)*10^(-(i-1)*logstep/4) xsol(4)*10^((i-1)*logstep/4) xsol(5)*10^(-(i-1)*logstep/2) xsol(6)*10^((i-1)*logstep/2)];
        else
    ...
    Dash_type = 1;
    figure(1)
    loglog(pI2,xsolall_dark(:,1),color{Dash_type,1},pI2,xsolall_dark(:,2),color{Dash_type,2},pI2,xsolall_dark(:,3),color{Dash_type,3},pI2,xsolall_dark(:,4),color{Dash_type,4},pI2,xsolall_dark(:,5),color{Dash_type,5},pI2,xsolall_dark(:,6),color{Dash_type,6})
...
xsol = [p0 n0 VI0 Ii0 VIx0 Iix0];

for i = 1:N 
%     i
    par = [pI2(i), kf_n_i, kf_p_i, kb_n_i, kb_p_i, kf_n_v, kf_p_v, kb_n_v, kb_p_v, kf_sg_i, kf_sg_v, kb_sg_i, kb_sg_v, KaF, kion_rec, ni, n1, p1, taun, taup, Gext, krad, Cn, Cp];
    fun = @(x)Equations_DarkLight(x, par, Rec_Model, Redox_Model);
...
figure(1)
hold on
loglog(pI2,xsolall(:,1),color{Dash_type,1},pI2,xsolall(:,2),color{Dash_type,2},pI2,xsolall(:,3),color{Dash_type,3},pI2,xsolall(:,4),color{Dash_type,4},pI2,xsolall(:,5),color{Dash_type,5},pI2,xsolall(:,6),color{Dash_type,6});
%Optionally plotting the initial guesses for diagnostics in case of poor
%convergence
% loglog(pI2,xguessall(:,1),'rx',pI2,xguessall(:,2),'bx',pI2,xguessall(:,3),'gx',pI2,xguessall(:,4),'kx')
xlabel('P(I_2) (bar)')
...
% figure(8)
% semilogx(pI2,Vth*log(xsolall_dark(:,6)./(xsolall_dark(:,6)+xsolall_dark(:,4))),'b.',pI2,Vth*log(xsolall_dark(:,5)./(xsolall_dark(:,5)+xsolall_dark(:,3))),'r.')

%Saving the data. The file Info contains the relevant input parameters
fid = fopen([NewDirectory, Filename,'_info.txt'],'wt');
Info = ['Suns = ' num2str(SunsEq) newline 'Gext = ' num2str(Gext*conc_) newline 'Rec_Model = ' num2str(Rec_Model) newline 'Redox_Model = ' num2str(Redox_Model)...
    newline 'KaF = ' num2str(KaF*conc_^2) ...
    newline 'Eg = ' num2str(Eg) newline 'NcNv = ' num2str(NcNv*conc_) newline 'ni  = ' num2str(ni*conc_) ...     
    newline 'kion_rec = ' num2str(kion_rec/conc_) newline 'PI2_IP = ' num2str(PI2_IP) newline 'PI2_NI  = ' num2str(PI2_NI) ...     
    newline 'PI2i = ' num2str(PI2i) newline 'Ki = ' num2str(K_sg_i/conc_) newline 'Kv  = ' num2str(K_sg_v*conc_) ...  
    newline 'K_p_i = ' num2str(K_p_i/conc_) newline 'K_n_i = ' num2str(K_n_i*conc_) ...
    newline 'K_p_v = ' num2str(K_p_v/conc_) newline 'K_n_v = ' num2str(K_n_v*conc_) ...
    newline 'vth = ' num2str(vth) ...
    newline 'Gamma_p_i' num2str(Gamma_p_i) newline 'Gamma_n_v' num2str(Gamma_n_v) newline 'Gamma_R' num2str(Gamma_p_i) ...
    newline 'Gamma_I_i = ' num2str(Gamma_I_i) newline 'Gamma_I_v = ' num2str(Gamma_I_v) ...
    newline 'kf_p_i = ' num2str(kf_p_i/conc_) newline 'kb_p_i = ' num2str(kf_p_i) ...
    newline 'kf_n_i = ' num2str(kf_n_i) newline 'kb_n_i = ' num2str(kb_n_i/conc_) ...
    newline 'kf_p_v = ' num2str(kf_p_v/conc_) newline 'kb_p_v = ' num2str(kf_p_v) ...
    newline 'kf_n_v = ' num2str(kf_n_v) newline 'kb_n_v = ' num2str(kb_n_v/conc_) ...
    newline 'K_sg_i = ' num2str(K_sg_i/conc_) newline 'K_sg_v = ' num2str(K_sg_v*conc_) ...
    newline 'kf_sg_i = ' num2str(kf_sg_i) newline 'kb_sg_i = ' num2str(kb_sg_i*conc_) ...
    newline 'kf_sg_v = ' num2str(kf_sg_v*conc_) newline 'kb_sg_v = ' num2str(kb_sg_v) ...
    newline 'taun = ' num2str(taun) newline 'taup = ' num2str(taup) ...
    newline 'n1 = ' num2str(n1*conc_) newline 'p1  = ' num2str(p1*conc_) ... 
    newline 'krad = ' num2str(krad/conc_) newline 'Cn = ' num2str(Cn/conc_^2) newline 'Cp = ' num2str(Cp/conc_^2) ...
