...
PI2_IP = 1e-2;                %PI2 at the boundary between the I and P region (p = [I_i^']). Estimate from conductivity measurements assuming mobility for n and p of 10 cm^2 V^-1 s^-1 and for ion 1e-9 cm^2 V^-1 s^-1
...
Info = ['Suns = ' num2str(SunsEq) newline 'Gext = ' num2str(Gext*conc_) ' cm^-3 s^-1' newline 'Rec_Model = ' num2str(Rec_Model) newline 'Redox_Model = ' num2str(Redox_Model)...
    ...
    newline 'kf_sg_i = ' num2str(kf_sg_i) ' s^-1' newline 'kb_sg_i = ' num2str(kb_sg_i*conc_) ' bar^-1/2 cm^-3 s^-1' ...
    newline 'kf_sg_v = ' num2str(kf_sg_v*conc_) ' cm^-3 s^-1' newline 'kb_sg_v = ' num2str(kb_sg_v) ' bar^-1/2 s^-1'...
